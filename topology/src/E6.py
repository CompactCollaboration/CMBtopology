...
class E6(Topology):
  def __init__(self, param, debug=True, make_run_folder = False):
    ...
    if np.linalg.norm(param['x0']) < 1e-6 and np.abs(param['r_x']-0.5) < 1e-6  and np.abs(param['r_y']-0.5) < 1e-6 and np.abs(param['r_z']-0.5) < 1e-6:
      self.no_shift = True
    else:
      self.no_shift = False
    ...
    self.root = 'runs/{}_LAx_{}_LBy_{}_LCz_{}_rx_{}_ry_{}_rz_{}_x_{}_y_{}_z_{}_l_max_{}_accuracy_{}_percent/'.format(
        param['topology'],
        "{:.2f}".format(param['Lx']),
        "{:.2f}".format(param['Ly']),
        "{:.2f}".format(param['Lz']),
...
@njit(parallel = False)
def get_list_of_k_phi_theta(k_max, L_Ax, L_By, L_Cz, r_x, r_y, r_z, x0):
    ...
    T_A = np.array([L_Ax,                   (r_y + 1/2)*L_By,   (r_z - 1/2) * L_Cz])
    T_B = np.array([(r_x - 1/2) * L_Ax, L_By,                   (r_z + 1/2)*L_Cz])
    T_C = np.array([(r_x + 1/2) * L_Ax, (r_y - 1/2) * L_By, L_Cz])
