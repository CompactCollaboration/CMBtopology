...
class E9(Topology):
  def __init__(self, param, debug=True, make_run_folder = False):
    ...
    self.V = abs(self.LAx * self.L1y * self.L2z)
...
def get_list_of_k_phi_theta(k_max, k_min, LAx, LAy, L1y, L2x, L2z, M_A, x0):
    ...
    n_z_max = int(np.ceil(k_max * abs(L2z) / (2*pi) + (n_x_max + n_y_max)/2)) 
    ...
    for n_x in range(-n_x_max, n_x_max+1):
      ...
      for n_y in range(1, n_y_max+1):
        ...
        for n_z in range(-n_z_max, n_z_max+1):
          k_z = 2 * pi / L2z * (n_z - L2x * n_x / (2 * LAx) - n_y / 2)
          ...
          phi[cur_index] = cur_phi
          theta[cur_index] = cur_theta
          tilde_xi[cur_index, 0] = exp(- 1j * dot(k_vec, x0))/sqrt(2) 
    ...
    print('E9 Final num of elements:', k_amp.size, 'Minimum k_amp', np.amin(k_amp), 'n_x_max', n_x_max, 'n_z_max', n_z_max)
          ...
          for m in range(-l, l + 1):
            ...
            for m_p in range(- l_p, l_p + 1):
              ...
              if m_p<0:
                ...
                sph_harm_l_m_p = sph_harm_no_phase[sph_harm_index, sph_p_cur_index] * phase_list_plus[abs_m_p] 
